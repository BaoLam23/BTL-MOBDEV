    ...
    public void check_is_clicked(float x, float y) {
        ...
        if (structureSymbol != null) {
            ...
            if(option.getYesbutton().get_is_clicked()){
              ...
              for(int i=structureSymbol.getImage().getPos().top ; i <= structureSymbol.getImage().getPos().bottom;i++){
                  for(int j=structureSymbol.getImage().getPos().left ; j <= structureSymbol.getImage().getPos().right;j++){
                      if(save_area[i][j] == 1){
                          could_build =false;
                      }
                  }
              ...
              if(could_build){
                  ...
                  // sau khi xây xong thì set lại vị trí trên map
                  for(int i=structureSymbol.getImage().getPos().top ; i <= structureSymbol.getImage().getPos().bottom;i++){
                      for(int j=structureSymbol.getImage().getPos().left ; j <= structureSymbol.getImage().getPos().right;j++){
                          save_area[i][j] = 1 ;
                      }
